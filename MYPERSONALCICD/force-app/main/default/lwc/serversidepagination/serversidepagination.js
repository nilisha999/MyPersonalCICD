...
export default class Serversidepagination extends LightningElement {
    ...
    // client side search
    handleSearchChangeClientSide(event) {
        // Step 1 : get all data in allRecords variable
        this.searchKey = event.target.value;
        let allRecords = this.data;
        let searchFilter = this.searchKey.toUpperCase();
        let tempArray = [];

        for (let i = 0; i < allRecords.length; i++) {
            if (allRecords[i][ACCOUNT_NAME_FIELD.fieldApiName] && allRecords[i][ACCOUNT_NAME_FIELD.fieldApiName].toUpperCase().indexOf(searchFilter) !== -1) {
