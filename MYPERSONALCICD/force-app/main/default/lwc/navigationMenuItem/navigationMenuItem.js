...
export default class NavigationMenuItem extends NavigationMixin(LightningElement) {
    ...
    handleClick(evt) {
        ...
        if (this.item.label === 'Logout') {
            // Kill Salesforce session and then redirect
            window.location.replace('/secur/logout.jsp?retUrl=/s/LoggedOut'); 
            // 🔹 retUrl is optional:
            // - In Communities → works (redirects to that relative URL).
            // - In core Salesforce org → ignored, you’ll land on login page.
            return;
        }
        ...
        if (this.pageReference) {
            ...
            console.error(
                `Navigation menu type "${this.item.type}" not implemented for item ${JSON.stringify(
                    this.item
                )}`
            );
        }
